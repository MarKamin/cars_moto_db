...
class Cars(db.Model):
    ...
    servisas = db.relationship("AutoTaisykla", backref="Autoservisas", lazy=True, cascade="all, delete-orphan")
    brand = db.Column("marke", db.String(66))
    model = db.Column("modelis", db.String(66))
    # year = db.Column("Metai", db.DateTime(datetime.date))
    year = db.Column("metai", db.Integer())
...
class AutoTaisykla(db.Model):
    ...
    def __init__(self, user_id, auto_id, pavadinimas, spec, rating):
        self.user_id = user_id
